...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if nc == 1:
            ...
            x[:, 7:12] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            conf1, j1 = x[:, 5:7].max(1, keepdim=True)
            conf2, j2 = x[:, 7:12].max(1, keepdim=True)
            conf = conf1*conf2
            j = j1*5+j2
            x = torch.cat((box, conf, j.float(), conf1, j1.float(), conf2, j2.float()), 1)[conf.view(-1) > conf_thres]
